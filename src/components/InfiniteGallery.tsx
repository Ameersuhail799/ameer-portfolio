...
const UPLOADED_GALLERY_IMAGES: GalleryImage[] = [
  { src: "/gallery/20260215_165950.jpg", alt: "Gallery photo 1" },
  { src: "/gallery/20260711_083143-1.jpg", alt: "Gallery photo 2" },
  { src: "/gallery/20260718_182456.jpg", alt: "Gallery photo 3" },
  { src: "/gallery/20260718_182457.jpg", alt: "Gallery photo 4" },
  { src: "/gallery/IMG_0793~2.jpg", alt: "Gallery photo 5" },
  { src: "/gallery/IMG_20250120_230044_404.webp", alt: "Gallery photo 6" },
  { src: "/gallery/IMG_20260718_181530.jpg", alt: "Gallery photo 7" },
  { src: "/gallery/IMG_20260718_181531.jpg", alt: "Gallery photo 8" },
  { src: "/gallery/PXL_20241222_120004880~6.jpg", alt: "Gallery photo 9" },
  { src: "/gallery/PXL_20241229_152253392~4.jpg", alt: "Gallery photo 10" },
  { src: "/gallery/ameerpic~2.jpeg", alt: "Gallery photo 11" },
  { src: "/gallery/retouch_2025120117281780.jpg", alt: "Gallery photo 12" },
  { src: "/gallery/retouch_2026040404284589.jpg", alt: "Gallery photo 13" },
  { src: "/gallery/retouch_2026040404312235.jpg", alt: "Gallery photo 14" },
  { src: "/gallery/retouch_2026052815510134.jpg", alt: "Gallery photo 15" },
  { src: "/gallery/retouch_2026052823172617.jpg", alt: "Gallery photo 16" },
];
...
const COMPONENT_DEFAULTS = {
  ...
  height: "100%",
  className: "",
  images: UPLOADED_GALLERY_IMAGES,
  density: 3,
  imageWidth: 220,
  imageHeight: 220,
  rounded: 12,
...
export default function InfiniteGallery(props: InfiniteGalleryProps) {
  ...
  const safeDensity = Math.max(1, Math.min(15, Math.floor(density || 6)));
  const safeImageWidth = Math.max(8, Math.min(4000, imageWidth || 200));
  const safeImageHeight = Math.max(8, Math.min(4000, imageHeight || 200));
  const safeRounded = Math.max(0, Math.min(20, rounded ?? 8));
  const safeDragSpeed = Math.max(0.1, Math.min(5, (dragSpeed || 20) / 20));
  const safeDriftAmount = Math.max(0, Math.min(20, driftAmount ?? 12));
  const safeFriction =
  ...
  useEffect(() => {
    ...
    const ensureTile = (t: Tile): HTMLDivElement => {
      ...
      if (!el) {
        ...
        img.style.boxShadow = "0 20px 40px -15px rgba(0,0,0,0.8)";
    ...
    const loop = () => {
      // Continuous ambient drift for gentle background movement
      const ambientX = 0.2;
      const ambientY = 0.1;
      const tx = targetX.get() + velX.get() + ambientX;
      const ty = targetY.get() + velY.get() + ambientY;
      targetX.set(tx);
      targetY.set(ty);
